...
public class MemberShipDTO {
    ...
    @Size(min = 2, max = 10, message = "이름은 두 글자에서 10글자 이내로 작성하시오 ex) 홍길동")
    ...
    @Size(min = 10, max = 16, message = "최소 10에서 최대 16 이내로 비밀번호를 작성하시오")
    private String password;    //비밀번호  // 낫 블링크,
                                // 최소 : 10, 최대 16 메시지 : 최소 10에서 최대 16 이내로 비밀번호를 작성하셔야 합니다.

    @NotBlank(message = "주소는 꼭 작성하셔야 합니다.")
    private String address; //주소   // 낫 블링크

    //권한
    private Role role;  // 이건 어차피 유저디테일서비스에서 직접 넣어줄꺼임   //필요에 따라 어드민 가입페이지와
                        // 일반유저 가입페이지가 분리될 수 있음
