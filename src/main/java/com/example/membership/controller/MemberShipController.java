...
public class MemberShipController {
    ...
    @GetMapping("/signup")
    public String siginup(MemberShipDTO memberShipDTO) {

        //파라미터는~~ 유효성검사를 함녀 다시 보내줄거라서~~~
        return "user/signup";
    }

    @PostMapping("/signup")
    public String siginup(@Valid MemberShipDTO memberShipDTO, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
    ...
    public String login(MemberShipDTO memberShipDTO, Principal principal) {
        ...
        if (principal != null) { //principal 로그인이 되었을때 값을 가지게 됩니다.
            // 현재는 email을 username으로 가졌기 때문에 로그인한 email을 가지고 있습니다.
            log.info("=======================");
